...
class bl3p(object):
    ...
    def balance(self, symbol):
        ...
        return float(
            int(balances["data"]["wallets"][symbol]["available"]["value_int"])
            * multiplier
        )
    ...
    def closed_order(self, id):
        ...
        if order["result"] == "success" and order["data"]["status"] == "closed":
            price = float(
                int(order["data"]["price"]["value_int"]) * self.PRICE_MULTIPLIER
            )
            volume = float(
                int(order["data"]["total_amount"]["value_int"]) * self.VOLUME_MULTIPLIER
            )
            fee = float(
                int(order["data"]["total_fee"]["value_int"]) * self.PRICE_MULTIPLIER
            )
    ...
    def call(self, path, data):
        ...
        signature = base64.b64encode(
            hmac.new(
                base64.b64decode(self.private_key), body.encode("utf-8"), hashlib.sha512
            ).digest()
        )
