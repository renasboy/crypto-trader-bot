...
class cobinhood:
    ...
    def start_listener(self, on_msg, on_err, on_open, on_close):
        def subscribe():
            message = (
                '{ "action": "subscribe", "trading_pair_id": "'
                + self.symbol
                + '", "type": "trade" }'
            )
            ws.send(message)
    ...
    def orders(self):
        orders = self.call(
            "get", "/v1/trading/orders", {"trading_pair_id": self.symbol}
        )
        return [
            order
            for order in orders["result"]["orders"]
            if order["state"] in ("open", "new", "partially_filled")
        ]
